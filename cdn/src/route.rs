...
pub async fn get_blob(
        ...
        .await?
    {
        ...
        None => {
            ...
            let blob_data =
                get_blob_from_pds(pds_endpoint.clone(), did.clone(), cid.clone()).await?;
            verify_cid(cid, &blob_data.clone())?;
            let img = read_image(&blob_data)?;
            let img = preset.process(img)?;

            let out_bytes =
                get_image_bytes(img, format.clone()).map_err(|_| CdnError::InvalidImage)?;

            app_state
                .aws()
                .store_blob(cache_params, out_bytes.clone())
                .await?;
    ...
    headers.insert("Content-Type", format.to_mime_type().parse().unwrap());
